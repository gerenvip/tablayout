 ...
 * @attr ref com.google.android.material.R.styleable#TabLayout_vTabMinHeight
 * @attr ref com.google.android.material.R.styleable#TabLayout_vTabMaxHeight
...
public class VerticalTabLayout extends ScrollView {
   ...
   * drawable's intrinsic width (preferred), via the {@code tabIndicatorWidth} attribute
   * (deprecated), or via {@link #setSelectedTabIndicatorWidth(int)} (deprecated). Otherwise, the
   * indicator will not be shown. This is the default value.
   *
   * @attr ref R.styleable#TabLayout_vTabIndicatorGravity
   * @see #setSelectedTabIndicatorGravity(int)
   * @see #getTabIndicatorGravity()
   */
  public static final int INDICATOR_GRAVITY_RIGHT = 0;
   ...
   * @see #getTabIndicatorGravity()
   */
  public static final int INDICATOR_GRAVITY_LEFT = 2;

  /**
   * Indicator gravity used to stretch the tab selection indicator across the entire height and
   * width of the {@link VerticalTabLayout}. This will disregard {@code tabIndicatorWidth} and the
   ...
   * 紧贴 tab 内容右边界
   */
  public static final int INDICATOR_GRAVITY_CONTENT_RIGHT = 4;
  /**
   * 紧贴 tab 内容左边界
   */
  public static final int INDICATOR_GRAVITY_CONTENT_LFT = 5;
  ...
  @IntDef(
          value = {
                  ...
                  INDICATOR_GRAVITY_CONTENT_RIGHT,
                  INDICATOR_GRAVITY_CONTENT_LFT
          })
  @Retention(RetentionPolicy.SOURCE)
  ...
  int mode;
  boolean inlineLabel;
  boolean tabIndicatorFullHeight;
  boolean unboundedRipple;

  //TabLayout custom
  int minIndicatorHeight;
  ...
  public VerticalTabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    slidingTabIndicator.setSelectedIndicatorWidth(
            a.getDimensionPixelSize(R.styleable.TabLayout_vTabIndicatorWidth, -1));
    ...
    if (a.hasValue(R.styleable.TabLayout_vTabIndicatorGravity)) {
      setSelectedTabIndicatorGravity(a.getInt(R.styleable.TabLayout_vTabIndicatorGravity, INDICATOR_GRAVITY_RIGHT));
    } else {
      setSelectedTabIndicatorGravity(
              a.getInt(com.google.android.material.R.styleable.TabLayout_tabIndicatorGravity, INDICATOR_GRAVITY_RIGHT));
    }
    setTabIndicatorFullHeight(a.getBoolean(R.styleable.TabLayout_vTabIndicatorFullHeight, true));
    ...
    requestedTabMinHeight =
            a.getDimensionPixelSize(R.styleable.TabLayout_vTabMinHeight, INVALID_HEIGHT);
    requestedTabMaxHeight =
            a.getDimensionPixelSize(R.styleable.TabLayout_vTabMaxHeight, INVALID_HEIGHT);
    ...
    // TabLayout define
    minIndicatorHeight = a.getDimensionPixelSize(R.styleable.TabLayout_vMinIndicatorHeight, (int) ViewUtils.dpToPx(getContext(), MIN_INDICATOR_HEIGHT));
    ...
    tabIndicatorVerticalPadding = a.getDimensionPixelOffset(R.styleable.TabLayout_vTabIndicatorVerticalPadding, 0);
    ...
    scrollableTabMinHeight = res.getDimensionPixelSize(R.dimen.design_tab_scrollable_min_height);

    // Now apply the tab mode and gravity
    applyModeAndGravity();

    indicatorTopInterpolator = new IndicatorInterpolator(true);
    indicatorBottomInterpolator = new IndicatorInterpolator(false);
  }

  /**
   * 设置 Indicator 变化的 插值器，仅在 {@link #setTabIndicatorSticky(boolean)}为 true 的情形下生效
   * </p>
   * 否则 默认情况下是 线性的 <br/>
   * 可参考效果设置如下:  <p/>
   * leftInterpolator: new AccelerateInterpolator()
   * rightInterpolator: new DecelerateInterpolator(1.25f)
   *
   * @param leftInterpolator  指示器 left 变化的 插值器
   * @param rightInterpolator 指示器 right 变化的插值器
   */
  public void setIndicatorInterpolator(Interpolator leftInterpolator, Interpolator rightInterpolator) {
    indicatorTopInterpolator = leftInterpolator;
    indicatorBottomInterpolator = rightInterpolator;
  }

  /**
   * 设置 定长指示器  {@link #setTabIndicatorFullHeight(boolean)} 为 false 的情况下生效。
   * <p/>
   * 建议 通过 R.styleable#TabLayout_tabIndicator 设置一个位图替换该方式
   *
   * @param indicatorFixedHeight 指示器的固定高度，默认值 -1 ，表示启用定长指示器。请设置一个非0 的正数
   * @attr ref R.styleable#TabLayout_vTabIndicatorFixedHeight
   */
  public void setTabIndicatorFixedHeight(int indicatorFixedHeight) {
    tabIndicatorFixedHeight = indicatorFixedHeight;
  }
  ...
  /**
   * 设置 tab 的指示器在水平方向的padding， 只在 {@link #setTabIndicatorFullHeight(boolean)} 为 false 的情况下生效
   *
   * @param padding 水平方向上的padding值，左右offset 是 padding 的一半
   * @attr ref R.styleable#TabLayout_vTabIndicatorFullHeight
  ...
  /**
   * set minIndicatorHeight
   *
   * @param minIndicatorHeight 当 tabIndicatorFullHeight 为 false，即 指示器的宽度由内容的宽度来决定 时
   *                           定义最小的指示器宽度，默认是 24dp
   * @attr ref R.styleable#TabLayout_vMinIndicatorHeight
   */
  public void setMinIndicatorHeight(int minIndicatorHeight) {
    this.minIndicatorHeight = minIndicatorHeight;
  }

  /**
   * get minIndicatorHeight
   */
  public int getMinIndicatorHeight() {
    return minIndicatorHeight;
  }
  ...
  /**
   * Sets the tab indicator's width for the currently selected tab. This method is deprecated. If
   * possible, set the intrinsic width directly on a custom indicator drawable passed to {@link
   * #setSelectedTabIndicator(Drawable)}.
   *
   * @param width width to use for the indicator in pixels
   * @attr ref R.styleable#TabLayout_vTabIndicatorWidth
   */
  @Deprecated
  public void setSelectedTabIndicatorWidth(int width) {
    slidingTabIndicator.setSelectedIndicatorWidth(width);
  }
  ...
  /**
   * Add a {@link VerticalTabLayout.OnTabSelectedListener} that will be invoked when tab selection changes.
   *
  ...
  /**
   * Add a {@link VerticalTabLayout.BaseOnTabSelectedListener} that will be invoked when tab selection
   * changes.
   *
   * <p>Components that add a listener should take care to remove it when finished via {@link
  ...
  /**
   * Remove the given {@link VerticalTabLayout.OnTabSelectedListener} that was previously added via {@link
  ...
  /**
   * Remove the given {@link VerticalTabLayout.BaseOnTabSelectedListener} that was previously added via
  ...
  /**
   * Remove all previously added {@link VerticalTabLayout.OnTabSelectedListener}s.
   */
   ...
   * defaults to {@link #INDICATOR_GRAVITY_RIGHT} if not set.
   * <p>
   ...
   *
   * @param indicatorGravity one of {@link #INDICATOR_GRAVITY_RIGHT}, {@link
   *                         #INDICATOR_GRAVITY_CENTER}, {@link #INDICATOR_GRAVITY_LEFT},{@link
   *                         #INDICATOR_GRAVITY_STRETCH},{@link #INDICATOR_GRAVITY_CONTENT_RIGHT},
   *                         or {@link #INDICATOR_GRAVITY_CONTENT_LFT}
   ...
   * VerticalTabLayout}.
   *
   * @return one of {@link #INDICATOR_GRAVITY_RIGHT}, {@link #INDICATOR_GRAVITY_CENTER}, {@link
   * #INDICATOR_GRAVITY_LEFT}, or {@link #INDICATOR_GRAVITY_STRETCH}
   */
  @TabIndicatorGravity
   ...
   *
   * @param tabIndicatorFullHeight Whether or not to fit selection indicator width to full width of
   *                               the tab item
   * @attr ref com.google.android.material.R.styleable#TabLayout_tabIndicatorFullWidth
   * @see #isTabIndicatorFullHeight()
   */
  public void setTabIndicatorFullHeight(boolean tabIndicatorFullHeight) {
    this.tabIndicatorFullHeight = tabIndicatorFullHeight;
    ViewCompat.postInvalidateOnAnimation(slidingTabIndicator);
  }

  /**
   * Get whether or not selection indicator width is fit to full height of the tab item, or fit to
   * the tab item's content.
   *
   * @return whether or not selection indicator width is fit to the full width of the tab item
   * @attr ref com.google.android.material.R.styleable#TabLayout_tabIndicatorFullWidth
   * @see #setTabIndicatorFullHeight(boolean)
   */
  public boolean isTabIndicatorFullHeight() {
    return tabIndicatorFullHeight;
  }
  ...
  private TabView createTabView(@NonNull final Tab tab) {
    ...
    tabView.setMinimumWidth(getTabMinHeight());
   ...
   * @param tab                 The tab to select, or {@code null} to select none.
   * @param updateIndicator     Whether to animate to the selected tab.
   * @param dispatchTabSelected Whether to dispatch tab select change to register listener
   * @see #selectTab(Tab)
   */
  public void selectTab(@Nullable final Tab tab, boolean updateIndicator, boolean dispatchTabSelected) {
  ...
  private void applyModeAndGravity() {
    ...
    switch (mode) {
      ...
      case MODE_FIXED:
        ...
        break;
      case MODE_SCROLLABLE:
        slidingTabIndicator.setGravity(Gravity.TOP);
        break;
    }
  ...
  public final class TabView extends LinearLayout {
    ...
    protected void drawableStateChanged() {
      ...
      if (changed) {
        invalidate();
        VerticalTabLayout.this.invalidate(); // Invalidate TabLayout, which draws mBaseBackgroundDrawable
      }
    }
    ...
    public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
      ...
      final int maxWidth = getTabMaxHeight();
      ...
      if (maxWidth > 0 && (specWidthMode == MeasureSpec.UNSPECIFIED || specWidthSize > maxWidth)) {
        ...
        widthMeasureSpec = MeasureSpec.makeMeasureSpec(tabMaxHeight, MeasureSpec.AT_MOST);
      } else {
  ...
  private class SlidingTabIndicator extends LinearLayout {
    ...
    private void updateIndicatorPosition() {
      ...
      if (selectedTitle != null && selectedTitle.getHeight() > 0) {
        ...
        if (!tabIndicatorFullHeight && selectedTitle instanceof TabView) {
        ...
        if (selectionOffset > 0f && selectedPosition < getChildCount() - 1) {
          ...
          if (!tabIndicatorFullHeight && nextTitle instanceof TabView) {
          ...
          if (tabIndicatorSticky) {
            float topFactor = indicatorTopInterpolator.getInterpolation(selectionOffset);
            float bottomFactor = indicatorBottomInterpolator.getInterpolation(selectionOffset);
            top = (int) (topFactor * nextTitleTop + (1.0f - topFactor) * top);
            bottom = (int) (bottomFactor * nextTitleBottom + (1.0f - bottomFactor) * bottom);
          } else {
    ...
    void animateIndicatorToPosition(final int position, int duration) {
      ...
      if (!tabIndicatorFullHeight && targetView instanceof TabView) {
    ...
    private void calculateTabViewContentBounds(
      ...
      @SuppressLint("RestrictedApi") int minIndicatorHeight = VerticalTabLayout.this.minIndicatorHeight;

      tabViewContentHeight -= tabIndicatorVerticalPadding;
      ...
      int tabViewHorizontalCenter = (tabView.getLeft() + tabView.getRight()) / 2;
      int contentLeftBounds = tabViewHorizontalCenter - (tabViewContentWidth / 2);
      int contentRightBounds = tabViewHorizontalCenter + (tabViewContentWidth / 2);

      if (tabIndicatorFixedHeight > 0) {
        int fixedHeight = tabIndicatorFixedHeight;
        if (tabIndicatorFixedHeight > tabView.getHeight()) {
    ...
    public void draw(@NonNull Canvas canvas) {
      ...
      if (tabSelectedIndicator != null) {
        indicatorWidth = tabSelectedIndicator.getIntrinsicWidth();
      }
      ...
      switch (tabIndicatorGravity) {
        case INDICATOR_GRAVITY_RIGHT:
          int right = getWidth() - tabIndicatorHorizontalPadding;
          indicatorLeft = right - indicatorWidth;
          indicatorRight = right;
          break;
        case INDICATOR_GRAVITY_CENTER:
          indicatorLeft = (getWidth() - indicatorWidth) / 2 + tabIndicatorHorizontalPadding;
          indicatorRight = indicatorLeft + indicatorWidth;
          break;
        case INDICATOR_GRAVITY_LEFT:
          indicatorLeft = tabIndicatorHorizontalPadding;
          indicatorRight = indicatorLeft + indicatorWidth;
          break;
        case INDICATOR_GRAVITY_STRETCH:
          indicatorLeft = tabIndicatorHorizontalPadding;
          indicatorRight = getWidth() - tabIndicatorHorizontalPadding;
          break;
        case INDICATOR_GRAVITY_CONTENT_RIGHT:
          int contentRight = (int) tabViewContentBounds.right;
          indicatorLeft = contentRight + tabIndicatorHorizontalPadding;
          indicatorRight = indicatorLeft + indicatorWidth;
          break;
        case INDICATOR_GRAVITY_CONTENT_LFT:
          int contentLeft = (int) tabViewContentBounds.left;
          indicatorRight = contentLeft - tabIndicatorHorizontalPadding;
      ...
      if (this.indicatorTop >= 0 && this.indicatorBottom > this.indicatorTop) {
  ...
  public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
    ...
    public TabLayoutOnPageChangeListener(VerticalTabLayout tabLayout) {
